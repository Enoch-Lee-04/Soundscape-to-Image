...
class LearnedSinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        self.weights.data = self.weights.data.to(device)  # 确保 self.weights 在同一个设备上
        x = rearrange(x, 'b -> b 1')
...
class Unet(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # time conditioning
        time = time.to(device)
        self.to_time_hiddens = self.to_time_hiddens.to(device)  # 确保 self.to_time_hiddens 在同一个设备上
        time_hiddens = self.to_time_hiddens(time)

        # derive time tokens
        time_hiddens = time_hiddens.to(device)
        self.to_time_tokens = self.to_time_tokens.to(device)
        time_tokens = self.to_time_tokens(time_hiddens)
        time_hiddens = time_hiddens.to(device)
        self.to_time_cond = self.to_time_cond.to(device)
        ...
        if exists(text_embeds) and self.cond_on_text:
            ...
            self.text_to_cond = self.text_to_cond.to(device)
...
class Imagen(nn.Module):
    ...
    def p_losses(
        ...
        **kwargs
        ...
        pred_most_image = noise_scheduler.predict_start_from_noise(x_noisy, t = times, noise = pred)
        return pred_most_image,pred,losses.mean(),x_noisy,times
    ...
    def generate_images(self, 
                        ...
                        cond_scale=1.,
                        lowres_sample_noise_level = None,
                        use_unet1_only=False):  # 新增参数用于控制模式
        device = self.device
        text_embeds = text_embeds.to(device)

        # 保证 batch_size 和 text_embeds 的一致性
        batch_size = text_embeds.shape[0]

        lowres_cond_img = None  # 初始化低分辨率条件图像
        lowres_noise_times = None
        # 如果使用 unet1 采样模式
        if use_unet1_only:
            unet = self.unets[0]  # 只使用第一个UNet
            unet_cond_scale = cond_scale
            shape = (batch_size, self.channels, self.image_sizes[0], self.image_sizes[0])

            # 获取对应的 noise_scheduler
            noise_scheduler = self.noise_schedulers[0]  # 对应第一个 UNet 的 noise scheduler
            ...
            img = self.p_sample_loop(
                ...
                text_embeds=text_embeds,
                cond_scale=unet_cond_scale,
                noise_scheduler=noise_scheduler  # 传递 noise_scheduler
            )
            return img
        ...
        # 如果使用级联UNet模式
            ...
            num_unets = len(self.unets)  # 确保不超过已有的UNet数量
            cond_scale = cast_tuple(cond_scale, num_unets)  # 处理条件缩放

            for unet_idx in range(num_unets):
                unet = self.unets[unet_idx]  # 当前使用的 UNet
                unet = unet.to(device)  # 确保 UNet 在同一个设备上
                unet_cond_scale = cond_scale[unet_idx]
                image_size = self.image_sizes[unet_idx]
                shape = (batch_size, self.channels, image_size, image_size)
                # 获取对应的 noise_scheduler
                noise_scheduler = self.noise_schedulers[unet_idx]  # 对应当前 UNet 的 noise scheduler
                if unet_idx == 0:
                    img = self.p_sample_loop(
                        unet,
                        shape,
                ...
                if unet_idx >0:
                    lowres_noise_times = self.lowres_noise_schedule.get_times(batch_size, lowres_sample_noise_level, device = device)
                    lowres_noise_times = lowres_noise_times.to(device)
                        # 调整图片大小
                    ...
                    img = self.p_sample_loop(
                        ...
                        cond_scale=unet_cond_scale,
                        noise_scheduler=noise_scheduler,  # 传递 noise_scheduler
                        lowres_cond_img=lowres_cond_img,  # 传递前一个 UNet 生成的低分辨率图像
                        lowres_noise_times=lowres_noise_times  # 传递低分辨率噪声时间
                    )
        #返回值应该是tresor格式
            return img
    @torch.no_grad()
