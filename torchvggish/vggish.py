...
class VGG(nn.Module):
    def __init__(self, features):
        ...
        self.embeddings = nn.Sequential(
            ...
            nn.LeakyReLU(True),
            # nn.ReLU(True),
            nn.Linear(4096, 128), #torch.randn(64, 256, 1024)
            nn.LeakyReLU(True),
            #nn.ReLU(True)
        )
...
class WLC(VGG):
    def __init__(self, urls, device=None, pretrained=False, preprocess=True, postprocess=True, progress=True):
        super().__init__(make_layers())
        if pretrained:
            state_dict = hub.load_state_dict_from_url(urls['vggish'], progress=progress)
            super().load_state_dict(state_dict)
    ...
    def forward(self, x, fs=None):
        # print(x)
        if self.preprocess:
            x,sr = self._preprocess(x, fs)
        x = x.to(self.device)
        x = self.model.forward(x)
        # x=self.em(x['embedding'])
        # if self.postprocess:
        #     x = self._postprocess(x)
        return x
